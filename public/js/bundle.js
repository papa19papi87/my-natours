...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiZ2F5aXNlbW9yZSIsImEiOiJjbDBjYmFoeDExMng0M2NxdTgzcmkyZHZ5In0.KNb8cFehgIHNH-4YCz9S5Q';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/gayisemore/cl0gpt18u000x15ql363j6mmh1' //   // center: [-118.113491, 34.111745],
    //   // zoom: 10,
    //   // interactive: false,
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "".concat(type.toUperCase(), " updated successfully! "));
            }
...
if ('Stripe' in window) {
  stripe = Stripe('pk_test_51Kg6g3FdosWjbtwzWMEaSVtuiVDE4IT7uAUrKJPb17jyFQlgQlwfVGjpyx5Kvr5eB4eELDGJOqo07elzpGcmx6iM00KGAS3cJs');
}
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return _axios.default.post("/api/v1/booking/checkout-session/".concat(tourId));

          case 3:
            session = _context.sent;
            console.log(session); // const session = await axios.post((window.location = session.data.url));
