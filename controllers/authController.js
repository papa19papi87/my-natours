...
const createSendToken = (user, statusCode, res) => {
  ...
  // res.setHeader('Set-Cookie', ['secure=mhm; HttpOnly', 'notSecure=yeah']);
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // check is user exist and password exist
  const user = await User.findOne({ email }).select('+password');
  if (!user || !(await user.correctPassword(password, user.password))) {
    return next(new AppError('Incorrect email or password', 401));
  }

  // check if everything ok, send token to
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) update changedpassword properties  for the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError(
        'you have entere a wrong password! please enter the correct password to continue',
        401
      )
    );
  }
